...
export default function QuizList() {
  ...
  const renderCategoryNode = (category, level = 0) => {
    ...
    return (
      <div key={categoryId} className="space-y-2" data-testid={`category-node-${categoryId}`}>
        <div className="rounded-2xl border border-gray-100 bg-white p-2" style={{ marginLeft: level ? `${level * 12}px` : 0 }}>
          {isEditing ? (
            ...
            <>
              <div className="flex items-center gap-2">
                <button type="button" onClick={() => setSelectedCategoryId(categoryId)} className={`min-w-0 flex-1 rounded-xl px-3 py-2 text-left text-sm font-black transition ${isSelected ? "bg-blue-600 text-white" : "bg-gray-50 text-gray-800 hover:bg-blue-50 hover:text-blue-700"}`}>
                  <span className="block truncate">{category.name}</span>
                  <span className={isSelected ? "text-xs text-blue-100" : "text-xs text-gray-500"}>{nodeQuizzes.length} quiz</span>
                </button>
                <button type="button" onClick={() => startEditCategory(category)} className="rounded-lg border border-gray-200 p-2 text-gray-500 hover:text-blue-700" aria-label="Sửa nhóm"><Edit3 className="h-3.5 w-3.5" /></button>
                <button type="button" onClick={() => handleDeleteCategory(categoryId)} className="rounded-lg border border-red-100 p-2 text-red-500 hover:bg-red-50" aria-label="Xóa nhóm"><Trash2 className="h-3.5 w-3.5" /></button>
              </div>
              {nodeQuizzes.length > 0 && (
                <div className="mt-2 space-y-1 pl-3">
                  {nodeQuizzes.slice(0, 5).map(quiz => (
                    <button key={getQuizId(quiz)} type="button" onClick={() => { setSelectedCategoryId(categoryId); setSearchTerm(quiz.title || ""); }} className="block w-full truncate rounded-lg bg-gray-50 px-2 py-1 text-left text-xs font-semibold text-gray-600 hover:bg-blue-50 hover:text-blue-700">
      ...
      <div className="relative z-10 mx-auto max-w-7xl px-6 py-10">
        ...
        <div className="grid gap-6 lg:grid-cols-[20rem_1fr]">
          <aside className="space-y-4">
            <Card className="rounded-3xl border-2 border-gray-200 bg-white shadow-lg">
              <CardContent className="p-4">
                <div className="mb-4 flex items-center gap-2"><FolderTree className="h-5 w-5 text-blue-700" /><div><h2 className="text-lg font-black text-gray-900">Cây phân loại</h2><p className="text-xs font-semibold text-gray-500">Cha &gt; con &gt; quiz</p></div></div>
                <div className="mb-4 grid grid-cols-2 gap-2">
                  <button type="button" onClick={() => setSelectedCategoryId("all")} className={`rounded-2xl px-3 py-2 text-sm font-black ${selectedCategoryId === "all" ? "bg-blue-600 text-white" : "bg-gray-100 text-gray-700"}`}>Tất cả</button>
                  <button type="button" onClick={() => setSelectedCategoryId("uncategorized")} className={`rounded-2xl px-3 py-2 text-sm font-black ${selectedCategoryId === "uncategorized" ? "bg-blue-600 text-white" : "bg-gray-100 text-gray-700"}`}>Chưa phân loại</button>
                </div>
                <div className="mb-4 space-y-2 rounded-2xl border border-gray-100 bg-gray-50 p-3">
